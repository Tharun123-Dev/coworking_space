...
const RECENT_ACTIVITIES = [
  ...
  { id:7,  type:"owner",    icon:IC.owners,   title:"Manager account created",       desc:"New owner Rahul K. onboarded for Gachibowli branch",                   time:"2 hr ago",   color:"#f59e0b", badge:"Owner"    },
          ...
          <div className={styles.ownerFormSection}>
            ...
            <div className={styles.ownerFormGrid}>
              ...
              <div className={styles.ownerFormField}>
                ...
                <select className={styles.ownerFormSelect} value={form.business_type} onChange={F("business_type")}>
          ...
          <div className={styles.ownerFormSection}>
            ...
            <div className={styles.ownerFormGrid}>
              ...
              <div className={styles.ownerFormField}>
                ...
                <select className={styles.ownerFormSelect} value={form.city} onChange={F("city")}>
          ...
          <div className={styles.ownerFormSection}>
            ...
            <div className={styles.ownerFormGrid}>
              <div className={styles.ownerFormField}>
                ...
                <select className={styles.ownerFormSelect} value={form.agreement_type} onChange={F("agreement_type")}>
                  <option value="lease">Lease Agreement</option>
                  <option value="license">Leave & License</option>
                  <option value="revenue_share">Revenue Share Agreement</option>
...
// ─── ADD WORKSPACE MODAL (step 1 — ask yes/no) ───────────────────────────────
function AddWorkspaceModal({ onYes, onNo, onClose }) {
  return (
    <div className={styles.modalOverlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={styles.modalBox}>
        ...
        <p className={styles.modalSub}>
          ...
          Fill their renting & revenue details now, or skip and add later.
        </p>
...
function UnifiedManagementPanel({ showToast, initialRoleFilter = "all" }) {
  ...
  const handleDeleteUser  = async (id) => { if(!window.confirm("Delete user?"))return; try{await axiosInstance.delete(`leads/users/delete/${id}/`);showToast("Deleted");fetchUsers();}catch{showToast("Failed","error");} };
  const handleCreateOwner = async () => { if(!ownerForm.username||!ownerForm.email||!ownerForm.password){showToast("Username, email and password required","error");return;} try{setLoading(true);await axiosInstance.post("admin/create-owner/",ownerForm);showToast("Manager created");resetForms();fetchOwners();}catch{showToast("Failed","error");}finally{setLoading(false);} };
  const handleUpdateOwner = async () => { try{setLoading(true);const p={username:ownerForm.username,email:ownerForm.email,location:ownerForm.location};if(ownerForm.password.trim())p.password=ownerForm.password;await axiosInstance.put(`owners/update/${editOwnerId}/`,p);showToast("Manager updated");resetForms();fetchOwners();}catch{showToast("Failed","error");}finally{setLoading(false);} };
  const handleDeleteOwner = async (id) => { if(!window.confirm("Delete manager?"))return; try{await axiosInstance.delete(`owners/delete/${id}/`);showToast("Deleted");fetchOwners();}catch{showToast("Failed","error");} };
  ...
  const S = {
    ...
    addUserBtn:  { display:"inline-flex",alignItems:"center",gap:"6px",padding:"8px 16px",borderRadius:"8px",border:"none",background:"#6366f1",color:"#fff",fontWeight:600,fontSize:"13px",cursor:"pointer" },
    addOwnerBtn: { display:"inline-flex",alignItems:"center",gap:"6px",padding:"8px 16px",borderRadius:"8px",border:"none",background:"#f59e0b",color:"#fff",fontWeight:600,fontSize:"13px",cursor:"pointer" },
    formCard:    (ac) => ({ background:"var(--card-bg,#fff)",border:`1.5px solid ${ac}30`,borderRadius:"12px",padding:"20px",marginBottom:"20px",boxShadow:`0 2px 12px ${ac}08` }),
    formTitle:   (ac) => ({ fontSize:"15px",fontWeight:700,color:ac,marginBottom:"16px",display:"flex",alignItems:"center",gap:"8px" }),
    formGrid:    { display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"12px",marginBottom:"16px" },
    inp:         { padding:"9px 12px",borderRadius:"8px",border:"1.5px solid var(--border,#e5e7eb)",fontSize:"13px",width:"100%",outline:"none",color:"#000",background:"#fff",boxSizing:"border-box" },
    ...
    submitBtn:   (c)  => ({ display:"inline-flex",alignItems:"center",gap:"6px",padding:"9px 18px",borderRadius:"8px",border:"none",background:c,color:"#fff",fontWeight:600,fontSize:"13px",cursor:"pointer" }),
    cancelBtn:   { display:"inline-flex",alignItems:"center",gap:"6px",padding:"9px 18px",borderRadius:"8px",border:"1.5px solid #000",background:"#fff",color:"#000",fontWeight:600,fontSize:"13px",cursor:"pointer" },
    filterBar:   { display:"flex",gap:"10px",marginBottom:"14px",flexWrap:"wrap",alignItems:"center" },
    searchWrap:  { display:"flex",alignItems:"center",gap:"8px",background:"var(--card-bg,#fff)",border:"1.5px solid var(--border,#e5e7eb)",borderRadius:"8px",padding:"7px 12px",flex:"1",minWidth:"200px" },
    searchInp:   { border:"none",outline:"none",fontSize:"13px",color:"black",width:"100%",background:"transparent" },
    roleBtn:     (a)  => ({ padding:"7px 14px",borderRadius:"8px",border:a?"none":"1.5px solid var(--border,#e5e7eb)",background:a?"#6366f1":"transparent",color:a?"#fff":"#555",fontWeight:600,fontSize:"12px",cursor:"pointer" }),
    tableWrap:   { overflowX:"auto",borderRadius:"12px",border:"1.5px solid var(--border,#e5e7eb)" },
    table:       { width:"100%",borderCollapse:"collapse",fontSize:"13px" },
    th:          { padding:"11px 14px",textAlign:"left",color:"var(--muted,#252020)",fontWeight:600,fontSize:"11px",textTransform:"uppercase",letterSpacing:"0.04em",borderBottom:"1.5px solid var(--border,#e5e7eb)",whiteSpace:"nowrap" },
    td:          { padding:"11px 14px",borderBottom:"1px solid var(--border-light,#f3f4f6)",color:"#000",verticalAlign:"middle" },
    avatarCell:  (c)  => ({ width:32,height:32,borderRadius:"50%",background:`${c}20`,color:c,display:"inline-flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:"13px",flexShrink:0 }),
    userBadge:   { display:"inline-flex",alignItems:"center",gap:"4px",padding:"3px 9px",borderRadius:"20px",background:"#6366f115",color:"#6366f1",fontSize:"11px",fontWeight:600,border:"1px solid #6366f125" },
    ownerBadge:  { display:"inline-flex",alignItems:"center",gap:"4px",padding:"3px 9px",borderRadius:"20px",background:"#f59e0b15",color:"#f59e0b",fontSize:"11px",fontWeight:600,border:"1px solid #f59e0b25" },
    adminBadge:  { display:"inline-flex",alignItems:"center",gap:"4px",padding:"3px 9px",borderRadius:"20px",background:"#10b98115",color:"#10b981",fontSize:"11px",fontWeight:600,border:"1px solid #10b98125" },
    editBtn:     { padding:"5px 10px",borderRadius:"6px",border:"none",background:"#6366f115",color:"#000",cursor:"pointer",fontSize:"12px",fontWeight:600,display:"inline-flex",alignItems:"center",gap:"4px" },
    delBtn:      { padding:"5px 10px",borderRadius:"6px",border:"none",background:"#f43f5e15",color:"#f43f5e",cursor:"pointer",fontSize:"12px",fontWeight:600,display:"inline-flex",alignItems:"center",gap:"4px" },
    emptyRow:    { padding:"40px",textAlign:"center",color:"var(--muted,#aaa)",fontSize:"14px" },
    countBadge:  { display:"inline-flex",alignItems:"center",padding:"3px 10px",borderRadius:"20px",background:"var(--badge-bg,#f3f4f6)",color:"var(--muted,#888)",fontSize:"11px",fontWeight:600,marginLeft:"8px" },
  };
  ...
  return (
    <div>
      ...
      <div style={S.actionBar}>
        <button style={{...S.addUserBtn,opacity:formMode==="addUser"?0.75:1}} onClick={()=>{resetForms();setFormMode(formMode==="addUser"?null:"addUser");}}>
          <Icon d={IC.userPlus} size={14}/>{formMode==="addUser"?"Close Form":"Add User"}
        </button>
        <button style={{...S.addOwnerBtn,opacity:formMode==="addOwner"?0.75:1}} onClick={()=>{resetForms();setFormMode(formMode==="addOwner"?null:"addOwner");}}>
          <Icon d={IC.owners} size={14}/>{formMode==="addOwner"?"Close Form":"Add Manager"}
        </button>
      </div>
      {isUserForm && (
        <div style={S.formCard("#6366f1")}>
          <div style={S.formTitle("#6366f1")}><Icon d={isEditMode?IC.edit:IC.userPlus} size={16}/>{isEditMode?`Edit User — ${userForm.username}`:"Add New User"}</div>
          <div style={S.formGrid}>
            ...
            {!isEditMode&&<input style={S.inp} placeholder="Password *" type="password" value={userForm.password} onChange={(e)=>setUserForm({...userForm,password:e.target.value})}/>}
          </div>
          {isEditMode&&<label style={S.checkRow}><input type="checkbox" checked={userForm.is_admin||false} onChange={(e)=>setUserForm({...userForm,is_admin:e.target.checked})} style={{width:16,height:16,accentColor:"#6366f1"}}/>Grant Admin Access</label>}
          <div style={S.formActions}>
            <button style={S.submitBtn("#6366f1")} disabled={loading} onClick={isEditMode?handleUpdateUser:handleCreateUser}><Icon d={isEditMode?IC.check:IC.add} size={13}/>{loading?"Saving...":isEditMode?"Update User":"Create User"}</button>
      ...
      {isOwnerForm && (
        <div style={S.formCard("#f59e0b")}>
          <div style={S.formTitle("#f59e0b")}><Icon d={isEditMode?IC.edit:IC.owners} size={16}/>{isEditMode?`Edit Manager — ${ownerForm.username}`:"Add New Manager"}</div>
          <div style={S.formGrid}>
            ...
            <select style={S.inp} value={ownerForm.location} onChange={(e)=>setOwnerForm({...ownerForm,location:e.target.value})}>
              ...
              {["Hitech City","Madhapur","Gachibowli","Kondapur","Financial District"].map((l)=><option key={l} value={l}>{l}</option>)}
            </select>
          ...
          <div style={S.formActions}>
            <button style={S.submitBtn("#f59e0b")} disabled={loading} onClick={isEditMode?handleUpdateOwner:handleCreateOwner}><Icon d={isEditMode?IC.check:IC.add} size={13}/>{loading?"Saving...":isEditMode?"Update Manager":"Create Manager"}</button>
            <button style={S.cancelBtn} onClick={resetForms}><Icon d={IC.close} size={13}/> Cancel</button>
          </div>
        </div>
      )}
      <div style={S.filterBar}>
        ...
        <div style={S.searchWrap}>
          ...
          {search&&<button style={{border:"none",background:"none",cursor:"pointer",color:"var(--muted,#aaa)",padding:0}} onClick={()=>setSearch("")}><Icon d={IC.close} size={12}/></button>}
        </div>
        <span style={S.countBadge}>{combined.length} records</span>
      </div>
      <div style={S.tableWrap}>
        <table style={S.table}>
          <thead><tr><th style={S.th}>#</th><th style={S.th}>Name</th><th style={S.th}>Email</th><th style={S.th}>Phone / Location</th><th style={S.th}>Role</th><th style={S.th}>Actions</th></tr></thead>
          <tbody>
            {combined.length===0
              ? <tr><td colSpan={6} style={S.emptyRow}>No records found.</td></tr>
              : combined.map((item,i)=>{
                  ...
                  return (
                    <tr key={`${item._type}-${item.id}`} style={{background:i%2===0?"transparent":"var(--row-alt,rgba(0,0,0,0.01))"}}>
                      <td style={{...S.td,fontSize:"12px"}}>{String(i+1).padStart(2,"0")}</td>
                      <td style={S.td}><div style={{display:"flex",alignItems:"center",gap:"10px"}}><div style={S.avatarCell(accent)}>{initial}</div><span style={{fontWeight:600}}>{item.username}</span></div></td>
                      <td style={{...S.td,fontSize:"12px"}}><div style={{display:"flex",alignItems:"center",gap:"5px"}}><Icon d={IC.mail} size={11}/>{item.email}</div></td>
                      <td style={{...S.td,fontSize:"12px"}}><div style={{display:"flex",alignItems:"center",gap:"5px"}}><Icon d={isOwner?IC.mapPin:IC.phone} size={11}/>{phoneOrLoc}</div></td>
                      <td style={S.td}>{isOwner?<span style={S.ownerBadge}><Icon d={IC.owners} size={10}/> Owner</span>:isAdmin?<span style={S.adminBadge}><Icon d={IC.shield} size={10}/> Admin</span>:<span style={S.userBadge}><Icon d={IC.users} size={10}/> User</span>}</td>
                      <td style={S.td}><div style={{display:"flex",gap:"6px"}}>
                        <button style={S.editBtn} onClick={()=>isOwner?openEditOwner(item):openEditUser(item)}><Icon d={IC.edit} size={12}/> Edit</button>
                        <button style={S.delBtn}  onClick={()=>isOwner?handleDeleteOwner(item.id):handleDeleteUser(item.id)}><Icon d={IC.trash} size={12}/> Delete</button>
                      </div></td>
                    </tr>
                  );
                })
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const sync=()=>{
      ...
      if(hs==="workspaces") setSection("workspaces");
      if(hq){ const p=new URLSearchParams(hq); const ow=p.get("owner"); if(ow){setOwnerFilter(ow);setSearchQ(ow);setSection("workspaces");} }
    };
    sync();
  ...
  const handleOwnerFormSubmit = (ownerData) => {
    ...
    setTimeout(() => {
      ...
      setShowAddForm(true);
      setOwnerFormLoading(false);
      showToast(`Owner "${ownerData.owner_name}" details saved — now fill workspace info`);
    }, 400);
  };
  ...
  const filteredWS = useMemo(() => {
    ...
    return workspaces.filter((w) => {
      const on  = (w.ownername||w.owner_name||w.owner?.username||w.owner?.name||"").toLowerCase();
      const wt  = (w.usertype||w.workspace_type||w.workspacetype||w.category||w.name||"").toLowerCase();
      const ms  = !q||w.name?.toLowerCase().includes(q)||w.city?.toLowerCase().includes(q)||w.location?.toLowerCase().includes(q)||on.includes(q)||wt.includes(q);
      const mo  = !ownerFilter||on===ownerFilter.toLowerCase();
      const mwt = !wsTypeFilter||wt===wsTypeFilter.toLowerCase();
      return ms&&mo&&mwt;
    });
  }, [workspaces,searchQ,ownerFilter,wsTypeFilter]);
  ...
  const sectionTitle = { overview:"Dashboard Overview", management:"User & Manager Management", workspaces:"Workspace Management", categories:"Category Management", tickets:"Support Tickets", activity:"Recent Activity" }[section] || "Dashboard";
  const activityTypes = ["all","booking","lead","ticket","workspace","user","payment","owner","category"];
  const wsSelectStyle = { padding:"0 10px",height:"36px",borderRadius:"8px",border:"1.5px solid var(--border,#e5e7eb)",fontSize:"12px",color:"#333",background:"#fff",outline:"none",cursor:"pointer",minWidth:"120px",maxWidth:"160px" };
            ...
            <section className={styles.overview}>
                ...
                {STATS.map((s,i)=>(
                  <button key={i} className={styles.statCard} style={{"--ac":s.color}} onClick={s.onClick} title={s.hint}>
                    <div className={styles.statTop}>
                      ...
                      <span className={`${styles.trend} ${s.up?styles.trendUp:styles.trendDn}`}><Icon d={s.up?IC.arrowUp:IC.arrowDown} size={10}/>{s.trend}%</span>
                    </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:"12px",marginBottom:"24px"}}>
                {[
                  ...
                  {label:"Manage Managers", icon:IC.owners,    color:"#f59e0b", hint:"View Managers only",    onClick:()=>{setManagementFilter("owners");setSection("management");}},
              ...
              <div className={styles.chartsRow}>
                ...
                <div className={styles.donutPanel}>
                  <div className={styles.panelHead}><h3 className={styles.panelTitle}>Space Mix</h3><span className={styles.panelSub}>By category type</span></div>
          ...
          {section==="management"&&(
            <section className={styles.section}>
              <div className={styles.secHead}>
                ...
                <div><h2 className={styles.secTitle}>User & Manager Management</h2><p className={styles.secSub}>Create, edit, delete users and managers.</p></div>
              </div>
          ...
          {section==="workspaces"&&(
            <section className={styles.section}>
              <div className={styles.secHead}>
                ...
                <div><h2 className={styles.secTitle}>Workspaces</h2><p className={styles.secSub}>Manage all workspaces, approvals, and owner assignments.</p></div>
                <span className={styles.countPill}>{filteredWS.length} Filtered</span>
              </div>

              {/* Toolbar */}
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px",flexWrap:"wrap"}}>
                <button onClick={handleAddWorkspaceBtnClick}
                  style={{display:"inline-flex",alignItems:"center",gap:"6px",height:"36px",padding:"0 14px",borderRadius:"8px",border:"none",background:showAddForm?"#f43f5e":"#f59e0b",color:"#fff",fontWeight:700,fontSize:"12px",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"background 0.18s"}}>
                ...
                <span style={{display:"inline-flex",alignItems:"center",padding:"0 10px",height:"36px",borderRadius:"20px",background:"var(--badge-bg,#f3f4f6)",color:"var(--muted,#888)",fontSize:"11px",fontWeight:600,whiteSpace:"nowrap",flexShrink:0}}>
          ...
          {section==="categories"&&(
            <section className={styles.section}>
              <div className={styles.secHead}>
                ...
                <div><h2 className={styles.secTitle}>Workspace Categories</h2><p className={styles.secSub}>Create and manage categories and pricing tiers.</p></div>
                <span className={styles.countPill}>{categories.length} Total</span>
              </div>
              <div className={styles.formCard}>
                <div className={styles.formHead}><span className={styles.formHeadIco}><Icon d={IC.add} size={13}/></span><span>Add New Category</span></div>
                <div className={styles.formGrid}>
